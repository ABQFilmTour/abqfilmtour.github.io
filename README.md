...
### Stretch goals
- Complete image submission and retrieval for individual locations. The database has the capability to store references to images for locations. Locations with an image attached will retrieve the image in the header, but it will only retrieve the first image and only if the first image is approved. We are in the process of integrating an image submission feature and a gallery view to view all of the images on a location.
